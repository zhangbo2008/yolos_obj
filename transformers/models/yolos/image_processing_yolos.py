...
def get_size_with_aspect_ratio(image_size, size, max_size=None) -> Tuple[int, int]:
    ...
    if max_size is not None:
        ...
        if max_original_size / min_original_size * size > max_size:#如果按照短边计算会超过长边, 那么我们就短边变小.进行四舍五入.
...
class YolosImageProcessor(BaseImageProcessor):
    ...
    def preprocess(
        ...
        **kwargs
        ...
        # transformations
        if do_resize:
            if annotations is not None:#对标注也进行reshape
    ...
    def post_process_object_detection(
        ...
        prob = nn.functional.softmax(out_logits, -1)
        scores, labels = prob[..., :-1].max(-1) #这行代码去掉最后一个分类,因为最后一个分类是beijign.
